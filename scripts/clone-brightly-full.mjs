/**
 * Clone template Brightly VIP từ tet.html - HTML đã render đầy đủ ảnh
 */

import fs from 'fs';

const SOURCE_FILE = "e:/wep/New folder (2)/ban co thiep moi/tet.html";
const OUTPUT_FILE = "scripts/imported-templates/content/mehappy-brightly-vip-3.json";

console.log("=== Clone Brightly VIP từ tet.html (HTML đầy đủ) ===\n");
...
// 1. Xóa React hydration scripts
cleaned = cleaned.replace(/<script[^>]*>[\s\S]*?self\.__next_f\.push\([\s\S]*?<\/script>/gi, "");
cleaned = cleaned.replace(/<script[^>]*>[\s\S]*?self\.__next_s\.push\([\s\S]*?<\/script>/gi, "");
cleaned = cleaned.replace(/<script[^>]*>[\s\S]*?__NEXT_DATA__[\s\S]*?<\/script>/gi, "");

// 2. Xóa script src
cleaned = cleaned.replace(/<script[^>]*\ssrc=["'][^"']*["'][^>]*><\/script>/gi, "");

// 3. Xóa external scripts
cleaned = cleaned.replace(/<script[^>]*src=["']https:\/\/connect\.facebook\.net[^"']*["'][^>]*><\/script>/gi, "");
cleaned = cleaned.replace(/<script[^>]*src=["']https:\/\/www\.googletagmanager\.com[^"']*["'][^>]*><\/script>/gi, "");
cleaned = cleaned.replace(/<script[^>]*id=["']google-analytics["'][^>]*>[\s\S]*?<\/script>/gi, "");
cleaned = cleaned.replace(/<script[^>]*id=["']meta-pixel["'][^>]*>[\s\S]*?<\/script>/gi, "");
...
// 9. Fix đường dẫn tương đối -> tuyệt đối
cleaned = cleaned.replace(/href="\.\//g, 'href="https://mehappy.vn/');
cleaned = cleaned.replace(/src="\.\//g, 'src="https://mehappy.vn/');

// 10. Fix Next.js image URLs
cleaned = cleaned.replace(/src="\/(_next\/image\?url=)([^&"]+)/g, (match, prefix, url) => {
  const decoded = decodeURIComponent(url);
  return `src="${decoded}"`;
});

// 11. Xóa các class không cần thiết
cleaned = cleaned.replace(/class="geist_[^"]*"/gi, "");
cleaned = cleaned.replace(/class="[^"]*module__[^"]*"/gi, "");

// 12. Wrap trong container
...
// Thêm template mới
const newTemplate = {
  id: "mehappy-brightly-vip-3",
  name: "Brightly VIP - Full HTML",
  description: "Template Brightly VIP - clone từ HTML đầy đủ ảnh",
  ...
  content_file: "mehappy-brightly-vip-3.json"
};
...
console.log("\n✅ Hoàn tất!");
console.log("   Template: Brightly VIP - Full HTML");
console.log("   Images: " + imgMatches.length);
